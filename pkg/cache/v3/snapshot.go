...
type Snapshot struct {
	...
	typeSnapshots [types.UnknownType]*types.TypeSnapshot
	mu            sync.RWMutex
}
...
// GetTypeSnapshot returns a `types.TypeSnapshot` instance to be used within new code instances.
// This implementation is less efficient (as allocating a map) than directly providing a new `types.Snapshot` instance.
func (s *Snapshot) GetTypeSnapshot(typeURL string) types.TypeSnapshot {
	...
	ts := types.NewTypeSnapshot(typeURL, s.GetVersion(typeURL), resources)
	s.mu.Lock()
	defer s.mu.Unlock()
	s.typeSnapshots[typ] = &ts
	return ts
}
