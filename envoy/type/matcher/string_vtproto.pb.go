...
func (m *StringMatcher) MarshalToSizedBufferVTStrict(dAtA []byte) (int, error) {
	...
	if msg, ok := m.MatchPattern.(*StringMatcher_Regex); ok {
		size, err := msg.MarshalToSizedBufferVTStrict(dAtA[:i])
		if err != nil {
			return 0, err
		}
...
func (m *StringMatcher_Regex) MarshalToVTStrict(dAtA []byte) (int, error) {
	size := m.SizeVT()
	return m.MarshalToSizedBufferVTStrict(dAtA[:size])
}

func (m *StringMatcher_Regex) MarshalToSizedBufferVTStrict(dAtA []byte) (int, error) {
	i := len(dAtA)
	i -= len(m.Regex)
	copy(dAtA[i:], m.Regex)
...
func (m *StringMatcher_Regex) SizeVT() (n int) {
	if m == nil {
		return 0
	}
	var l int
