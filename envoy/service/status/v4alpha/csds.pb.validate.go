...
import (
	...
	v4alpha "github.com/envoyproxy/go-control-plane/envoy/admin/v4alpha"
)
...
var (
	...
	_ = v4alpha.ClientResourceStatus(0)
)
...
func (m *ClientConfig) Validate() error {
	...
	for idx, item := range m.GetHiddenEnvoyDeprecatedXdsConfig() {
		_, _ = idx, item

		if v, ok := interface{}(item).(interface{ Validate() error }); ok {
			if err := v.Validate(); err != nil {
				return ClientConfigValidationError{
					field:  fmt.Sprintf("HiddenEnvoyDeprecatedXdsConfig[%v]", idx),
					reason: "embedded message failed validation",
					cause:  err,
				}
			}
