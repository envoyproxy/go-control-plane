...
func (m *InternalUpstreamTransport) validate(all bool) error {
	...
	for idx, item := range m.GetPassthroughFilterStateObjects() {
		_, _ = idx, item
		...
		if all {
			...
			case interface{ ValidateAll() error }:
				if err := v.ValidateAll(); err != nil {
					errors = append(errors, InternalUpstreamTransportValidationError{
						field:  fmt.Sprintf("PassthroughFilterStateObjects[%v]", idx),
						reason: "embedded message failed validation",
						cause:  err,
					})
				}
...
// Validate checks the field values on
// InternalUpstreamTransport_FilterStateSource with the rules defined in the
// proto definition for this message. If any rules are violated, the first
// error encountered is returned, or nil if there are no violations.
func (m *InternalUpstreamTransport_FilterStateSource) Validate() error {
	return m.validate(false)
}
...
// proto definition for this message. If any rules are violated, the result is
// a list of violation errors wrapped in
// InternalUpstreamTransport_FilterStateSourceMultiError, or nil if none found.
func (m *InternalUpstreamTransport_FilterStateSource) ValidateAll() error {
	return m.validate(true)
}

func (m *InternalUpstreamTransport_FilterStateSource) validate(all bool) error {
	if m == nil {
		return nil
	}
	...
	if utf8.RuneCountInString(m.GetName()) < 1 {
		err := InternalUpstreamTransport_FilterStateSourceValidationError{
			field:  "Name",
			reason: "value length must be at least 1 runes",
		}
		if !all {
	...
	if len(errors) > 0 {
		return InternalUpstreamTransport_FilterStateSourceMultiError(errors)
	}

	return nil
}

// InternalUpstreamTransport_FilterStateSourceMultiError is an error wrapping
// multiple validation errors returned by
// InternalUpstreamTransport_FilterStateSource.ValidateAll() if the designated
// constraints aren't met.
type InternalUpstreamTransport_FilterStateSourceMultiError []error

// Error returns a concatenation of all the error messages it wraps.
func (m InternalUpstreamTransport_FilterStateSourceMultiError) Error() string {
	var msgs []string
	for _, err := range m {
		msgs = append(msgs, err.Error())
	}
...
// AllErrors returns a list of validation violation errors.
func (m InternalUpstreamTransport_FilterStateSourceMultiError) AllErrors() []error { return m }

// InternalUpstreamTransport_FilterStateSourceValidationError is the validation
// error returned by InternalUpstreamTransport_FilterStateSource.Validate if
...
// Field function returns field value.
func (e InternalUpstreamTransport_FilterStateSourceValidationError) Field() string { return e.field }

// Reason function returns reason value.
func (e InternalUpstreamTransport_FilterStateSourceValidationError) Reason() string { return e.reason }

// Cause function returns cause value.
func (e InternalUpstreamTransport_FilterStateSourceValidationError) Cause() error { return e.cause }

// Key function returns key value.
func (e InternalUpstreamTransport_FilterStateSourceValidationError) Key() bool { return e.key }

// ErrorName returns error name.
func (e InternalUpstreamTransport_FilterStateSourceValidationError) ErrorName() string {
	return "InternalUpstreamTransport_FilterStateSourceValidationError"
}

// Error satisfies the builtin error interface
func (e InternalUpstreamTransport_FilterStateSourceValidationError) Error() string {
	cause := ""
	if e.cause != nil {
		cause = fmt.Sprintf(" | caused by: %v", e.cause)
	}
	...
	return fmt.Sprintf(
		"invalid %sInternalUpstreamTransport_FilterStateSource.%s: %s%s",
		key,
		e.field,
		e.reason,
		cause)
...
var _ error = InternalUpstreamTransport_FilterStateSourceValidationError{}
...
var _ interface {
	...
	Cause() error
	ErrorName() string
} = InternalUpstreamTransport_FilterStateSourceValidationError{}
