...
func (m *OAuth2Credentials_CookieNames) MarshalToSizedBufferVTStrict(dAtA []byte) (int, error) {
	...
	if len(m.OauthExpires) > 0 {
		...
		i = encodeVarint(dAtA, i, uint64(len(m.OauthExpires)))
		i--
		dAtA[i] = 0x1a
	}
	if len(m.OauthHmac) > 0 {
		...
		i = encodeVarint(dAtA, i, uint64(len(m.OauthHmac)))
		i--
		dAtA[i] = 0x12
	}
	if len(m.BearerToken) > 0 {
		...
		i = encodeVarint(dAtA, i, uint64(len(m.BearerToken)))
		i--
		dAtA[i] = 0xa
	}
...
func (m *OAuth2Config) MarshalToSizedBufferVTStrict(dAtA []byte) (int, error) {
	...
	if m.AuthType != 0 {
		i = encodeVarint(dAtA, i, uint64(m.AuthType))
		i--
		dAtA[i] = 0x58
	}
	...
	if len(m.AuthScopes) > 0 {
		for iNdEx := len(m.AuthScopes) - 1; iNdEx >= 0; iNdEx-- {
			...
			i = encodeVarint(dAtA, i, uint64(len(m.AuthScopes[iNdEx])))
			i--
			dAtA[i] = 0x4a
		}
	}
