...
func (m *CommonGeoipProviderConfig_GeolocationHeadersToAdd) MarshalToSizedBufferVTStrict(dAtA []byte) (int, error) {
	...
	if len(m.AnonProxy) > 0 {
		...
		i = encodeVarint(dAtA, i, uint64(len(m.AnonProxy)))
		i--
		dAtA[i] = 0x4a
	}
	if len(m.AnonTor) > 0 {
		...
		i = encodeVarint(dAtA, i, uint64(len(m.AnonTor)))
		i--
		dAtA[i] = 0x42
	}
	if len(m.AnonHosting) > 0 {
		...
		i = encodeVarint(dAtA, i, uint64(len(m.AnonHosting)))
		i--
		dAtA[i] = 0x3a
	}
	if len(m.AnonVpn) > 0 {
		...
		i = encodeVarint(dAtA, i, uint64(len(m.AnonVpn)))
		i--
		dAtA[i] = 0x32
	}
	if len(m.IsAnon) > 0 {
		...
		i = encodeVarint(dAtA, i, uint64(len(m.IsAnon)))
		i--
		dAtA[i] = 0x2a
	}
	if len(m.Asn) > 0 {
		i -= len(m.Asn)
		copy(dAtA[i:], m.Asn)
		i = encodeVarint(dAtA, i, uint64(len(m.Asn)))
		i--
		dAtA[i] = 0x22
	}
	if len(m.Region) > 0 {
