...
func (m *Cluster_QueuingPolicies) MarshalVTStrict() (dAtA []byte, err error) {
	if m == nil {
		return nil, nil
	}
	size := m.SizeVT()
...
func (m *Cluster_QueuingPolicies) MarshalToVTStrict(dAtA []byte) (int, error) {
	size := m.SizeVT()
	return m.MarshalToSizedBufferVTStrict(dAtA[:size])
}

func (m *Cluster_QueuingPolicies) MarshalToSizedBufferVTStrict(dAtA []byte) (int, error) {
	if m == nil {
		return 0, nil
	}
	i := len(dAtA)
	...
	if m.PendingRqPolicy != nil {
		if vtmsg, ok := interface{}(m.PendingRqPolicy).(interface {
			MarshalToSizedBufferVTStrict([]byte) (int, error)
		}); ok {
			size, err := vtmsg.MarshalToSizedBufferVTStrict(dAtA[:i])
			if err != nil {
			...
			encoded, err := proto.Marshal(m.PendingRqPolicy)
			if err != nil {
				return 0, err
			}
			i -= len(encoded)
...
func (m *Cluster) MarshalToSizedBufferVTStrict(dAtA []byte) (int, error) {
	...
	if m.QueuingPolicies != nil {
		size, err := m.QueuingPolicies.MarshalToSizedBufferVTStrict(dAtA[:i])
		if err != nil {
			return 0, err
		}
		i -= size
...
func (m *Cluster_QueuingPolicies) SizeVT() (n int) {
	if m == nil {
		return 0
	}
	var l int
	_ = l
	if m.PendingRqPolicy != nil {
		if size, ok := interface{}(m.PendingRqPolicy).(interface {
			SizeVT() int
		}); ok {
			l = size.SizeVT()
		} else {
			l = proto.Size(m.PendingRqPolicy)
		}
		n += 1 + l + protohelpers.SizeOfVarint(uint64(l))
	}
	n += len(m.unknownFields)
