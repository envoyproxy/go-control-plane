...
func (m *Tracing_Http) MarshalToSizedBufferVTStrict(dAtA []byte) (int, error) {
	...
	if msg, ok := m.ConfigType.(*Tracing_Http_Config); ok {
		size, err := msg.MarshalToSizedBufferVTStrict(dAtA[:i])
		if err != nil {
			return 0, err
		}
...
func (m *Tracing_Http_Config) MarshalToVTStrict(dAtA []byte) (int, error) {
	size := m.SizeVT()
	return m.MarshalToSizedBufferVTStrict(dAtA[:size])
}

func (m *Tracing_Http_Config) MarshalToSizedBufferVTStrict(dAtA []byte) (int, error) {
	i := len(dAtA)
	if m.Config != nil {
