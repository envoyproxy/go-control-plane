...
func (m *Language) validate(all bool) error {
	...
	if utf8.RuneCountInString(m.GetDefaultLanguage()) < 2 {
		err := LanguageValidationError{
	...
	if len(m.GetSupportedLanguages()) < 1 {
		err := LanguageValidationError{
	...
	_Language_SupportedLanguages_Unique := make(map[string]struct{}, len(m.GetSupportedLanguages()))

	for idx, item := range m.GetSupportedLanguages() {
		_, _ = idx, item

		if _, exists := _Language_SupportedLanguages_Unique[item]; exists {
			err := LanguageValidationError{
				field:  fmt.Sprintf("SupportedLanguages[%v]", idx),
			...
			_Language_SupportedLanguages_Unique[item] = struct{}{}
		}
